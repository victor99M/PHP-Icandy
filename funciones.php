...
function registrarProducto($data)
{
    ...
    ('$data->nombre', '$data->precio_publico', '$data->precio_inversion', '', '$data->cantidad', '$data->novedad', '$data->disponibilidad', '$data->descripcion_pro', CURRENT_DATE(), '$data->unidadPeso_PR', '$data->descripcion_pro', '$data->piezasCaja_PR')"; //'$data->url_imagen'
...
function insertarImagenProductos($foto)
{
    include("conexion.php");
    //$id_VP = mysqli_insert_id($conexion);
    $foto_aux = "https://icandymx.xyz/icandy/API/productos/" . $foto;

    $sql_id = "SELECT id_PR FROM producto order by id_PR desc limit 1";

    $consulta = mysqli_query($conexion, $sql_id);

    while ($row = $consulta->fetch_array()) {
        $id = $row['id_PR'];
    }

    $sql = "UPDATE producto set foto_PR = '{$foto_aux}' WHERE id_PR = '{$id}'";
